...
def main(args):
    embedding_clf_nominal, labels_clf_nominal = get_umap_embedding(args.preds_clf_nominal)
    ...
    for label_num, name in LABELS.items():
        embedding_label_clf_nominal = embedding_clf_nominal[(labels_clf_nominal == label_num)]
        ax[0].scatter(embedding_label_clf_nominal[:, 0], embedding_label_clf_nominal[:, 1], label=name, s=1)
        embedding_label_clf = embedding_clf[(labels_clf == label_num)]
        ax[1].scatter(embedding_label_clf[:, 0], embedding_label_clf[:, 1], label=name, s=1)
        embedding_label_clr = embedding_clr[(labels_clr == label_num)]
        ax[2].scatter(embedding_label_clr[:, 0], embedding_label_clr[:, 1], label=name, s=1)
        embedding_label_dann = embedding_dann[(labels_dann == label_num)]
        ax[3].scatter(embedding_label_dann[:, 0], embedding_label_dann[:, 1], label=name, s=1)

    for a in ax:
        a.set_xticks([])
        a.set_yticks([])
    ax[0].set_title("Classifier Nominal", fontsize=12)
...
def parse_arguments():
    ...
    parser.add_argument("preds_clf_nominal")
