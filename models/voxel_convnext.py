...
class VoxelConvNeXtSimCLR(nn.Module):
    """ Sparse ConvNeXtV2.
    ...
    def __init__(self, 
        ...
        for i in range(4):
            stage = nn.Sequential(
                *[BlockSimCLR(dim=dims[i], drop_path=dp_rates[cur + j], D=D) for j in range(depths[i])]
            )
            self.stages.append(stage)
            cur += depths[i]
...
#TODO fix this mess
class VoxelConvNeXtCLR(VoxelConvNeXtSimCLR):
